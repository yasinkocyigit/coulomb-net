...
def main():
    ...
    plot_learning_curve_with_ci(
        ...
        "Learning Curve (Random Forest)", X, y, axes=axes, cv=3)
    plt.savefig(save_dir / 'learning_curve_with_ci.png', dpi=150)
    plt.close()

    # Plot 2: Hexbin Density
    print("   - Generating Hexbin Density Plot...")
    plot_hexbin_density(y_test[:, 0], y_pred_rf[:, 0], 'q1', save_dir / 'hexbin_density_q1.png')
    
    # Plot 3: Radar Chart
    print("   - Generating Radar Chart...")
    radar_categories = ['R^2 Score', '1 - MSE (Normalized)', 'Speed (Simulated)']
    plot_radar_chart(metrics_data, radar_categories, "Model Comparison", save_dir / 'model_comparison_radar.png')

    # Plot 4: Contour Map
    print("   - Generating Contour Map...")
    # Using features r1_x and r1_y (indices 0 and 1) for the contour map
    feature_names = ThreeChargeDataset.feature_names()
    plot_contour_error_map(rf, X_test, y_test, [0, 1], feature_names, save_dir / 'contour_error_map.png')

    # Plot 5: ECDF of Residuals
    print("   - Generating ECDF of Residuals...")
    plot_ecdf_residuals(y_test, y_pred_rf, 'RandomForest', save_dir / 'ecdf_residuals.png')
